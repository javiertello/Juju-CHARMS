...
public class EventProducer {
    ...
    private void start(Context context) {
		...
		logger.info("Zookeeper: " + zk_array[0]);
	        ...
	        ZkClient zkClient = new ZkClient(zk_array[0], sessionTimeoutMs, connectionTimeoutMs, ZKStringSerializer$.MODULE$);

                // If topic don't exists must be created
                if(!AdminUtils.topicExists(zkClient, topic)){

		        int numPartitions = getPartitions(context.getString(EventSourceConstant.BROKER_LIST));
		        int replicationFactor = 1;

                        AdminUtils.createTopic(zkClient, topic, numPartitions, replicationFactor, new Properties());
		...
		props.put("producer.type", context.getString(EventSourceConstant.PRODUCER_TYPE));
                props.put("batch.num.messages", context.getString(EventSourceConstant.BATCH_SIZE));
		props.put("partitioner.class", "com.kafka.producer.RoundRobinPartitioner");
				...
				for(int i = 0; i < 2;i++){
					...
					if ( cuenta ==  8 ){
						...
						// Send it to kafka brokers
                                        	KeyedMessage<String, String> data = new KeyedMessage<String, String>(topic, Integer.toString(cuentaKey), envio);
                                        	producer.send(data);
						cuentaKey++;
                                        	//logger.info(s);
					}
